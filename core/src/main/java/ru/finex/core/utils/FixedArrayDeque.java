...
public class FixedArrayDeque<E> extends AbstractCollection<E> implements Deque<E>, Cloneable {
    ...
    public FixedArrayDeque(int numElements) {
        elements = new Object[(numElements < 1) ? 1 :
            numElements - MAX_ARRAY_SIZE > 0 ?
                MAX_ARRAY_SIZE : numElements + 1];
    }
     ...
     * @throws IllegalStateException if specification collection is biggest then queue
     */
     ...
     */
    public FixedArrayDeque<E> clone() {
        try {
            @SuppressWarnings("unchecked")
            FixedArrayDeque<E> result = (FixedArrayDeque<E>) super.clone();
